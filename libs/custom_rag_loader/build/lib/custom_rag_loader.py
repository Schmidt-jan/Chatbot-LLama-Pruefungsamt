...
class SupportedModels(Enum):
    ...
    Mistral_Finetune_Q4 = "mistralai/Mistral-7B-v0.3Q4"
    Mistral_Finetune_Huggingface_RAFT = "mistralai/Mistral-7B-v0.3v1"
    Mistral_Finetune_Huggingface_RAFTV2 = "mistralai/Mistral-7B-v0.3v2"
...
class RagConfig:
    ...
    def generate_model_path_and_db_path(self, 
        ...
        elif model_name == SupportedModels.Mistral_Finetune_Huggingface_RAFT:
            relative_model_path = "raft"
        
        elif model_name == SupportedModels.Mistral_Finetune_Huggingface_RAFTV2:
            relative_model_path = "raftv2"
...
def load_llm_huggingface_rag_model(config: RagConfig)-> Tuple[LlamaCpp, Chroma]:
    destroy_old_session()
    ...
    generation_config.temperature = 0.1 # randomness of the generated tex
...
def load_db(config: RagConfig) -> Chroma:
    if isinstance(config.db_embedding_model, SupportedModels):
        raise ValueError("Model not supported for database")

    if config.db_embedding_model.value.startswith("sentence-transformers"):
        embedding = SentenceTransformerEmbeddings(model_name=config.db_embedding_model.value.split("/")[-1])
