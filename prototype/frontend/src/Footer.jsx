...
const Footer = () => {
  ...
  const sendQuestionHandler = async (question) => {
    try {
      const { answer, pages, link } = await sendQuestion(question);
  ...
  const stream = async (params) => {
		try {
			...
			const response = await axios.get(`http://localhost:8000/requests?question=${params.userInput}`,{
        adapter: 'fetch',
        responseType: 'stream'
      }).then(async (response) => {
  ...
  const flow = {
    start: {
      message: "Hi, mein Name ist Timmy! Wie kann ich dir helfen?",
      path: "askQuestion"
    },
    askQuestion: {
